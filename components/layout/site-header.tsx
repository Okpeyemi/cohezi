    ...
    <header className="site-header sticky top-0 z-50 border-b border-transparent bg-ink transition-colors duration-300">
      <div className="mx-auto flex h-[68px] max-w-[1440px] items-center justify-between px-5 lg:px-24">
        ...
        <nav aria-label="Main" className="hidden items-center gap-6 lg:flex">
          {nav.map((item) => (
            <Link key={item.href} href={item.href} className="site-nav-link text-base font-medium text-white/90 transition-colors hover:text-white">
              {item.label}
            </Link>
          ))}
        </nav>
        <div className="hidden lg:block">
          <ButtonLink href={cta.href} variant="white" size="sm" className="border border-line">
            {cta.label}
