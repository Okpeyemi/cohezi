...
export function Hero({ hero }: { hero: HeroContent }) {
  return (
    <section aria-labelledby="hero-title" className="hero-dark-change px-5 pb-16 pt-20 text-center md:pt-28">
      <h1
        id="hero-title"
        className="mx-auto max-w-4xl text-4xl font-bold leading-[1.08] tracking-[-0.025em] text-paper md:text-[72px]"
      >
        {hero.titleStart} <br className="hidden md:inline" />
        <span className="text-brand-gradient">{hero.titleAccent}</span> {hero.titleEnd}
      </h1>
      <p className="mx-auto mt-6 max-w-[420px] text-lg leading-7 text-paper/80">{hero.subtitle}</p>
      <div className="mt-8 flex justify-center">
        <NewsletterForm variant="hero" placeholder={hero.emailPlaceholder} buttonLabel={hero.subscribeLabel} />
      </div>
      <p className="mt-16 text-sm text-paper/80">
        {hero.trustedByPrefix} <strong className="font-bold text-paper">{hero.trustedByCount}</strong> {hero.trustedBySuffix}
      </p>
